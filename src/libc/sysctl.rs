...
fn sysctl(
    ...
    for i in 0..namelen as usize {
        // FIXED: Using '+' and explicit read type to resolve Logs.txt errors
...
fn sysctlbyname(
    ...
    let name_str = String::from_utf8_lossy(env.mem.cstr_at(name_ptr));
    log!("GAMELOFT BYPASS: sysctlbyname '{}', faking hardware success", name_str);
    0
}

pub const FUNCTIONS: crate::dyld::FunctionExports = &[
    // FIXED: Macros now have correct underscore counts (matching args - 1)
