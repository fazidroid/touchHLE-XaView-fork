...
0x20000 } else { name }; // CacheKey
  ...
  0x8B8C => {
                ...
                gles11::EXTENSIONS => {
                    // SafeExtensionsEsTwo
                    if is_es2 {
                        // GAMELOFT BYPASS: Removed GL_IMG_texture_compression_pvrtc
                        b"GL_APPLE_framebuffer_multisample GL_APPLE_texture_max_level GL_EXT_discard_framebuffer GL_IMG_read_format GL_IMG_texture_format_BGRA8888 GL_OES_depth24 GL_OES_element_index_uint GL_OES_framebuffer_object GL_OES_packed_depth_stencil GL_OES_rgb8_rgba8 GL_OES_texture_mirrored_repeat GL_OES_vertex_half_float "
                    } else {
                        // GAMELOFT BYPASS: Removed GL_IMG_texture_compression_pvrtc
                        b"GL_APPLE_framebuffer_multisample GL_APPLE_texture_max_level GL_EXT_discard_framebuffer GL_EXT_texture_filter_anisotropic GL_EXT_texture_lod_bias GL_IMG_read_format GL_IMG_texture_format_BGRA8888 GL_OES_blend_subtract GL_OES_compressed_paletted_texture GL_OES_depth24 GL_OES_draw_texture GL_OES_framebuffer_object GL_OES_mapbuffer GL_OES_matrix_palette GL_OES_point_size_array GL_OES_point_sprite GL_OES_read_format GL_OES_rgb8_rgba8 GL_OES_texture_mirrored_repeat GL_OES_vertex_array_object "
                    }
                }
                _ => unreachable!(),
            };
...
fn glBindAttribLocation(env: &mut Environment, program: GLuint, index: GLuint, name: ConstVoidPtr) {
    with_ctx_and_mem(env, |gles, mem| unsafe {
...
fn glVertexAttrib3f(env: &mut Environment, indx: GLuint, x: GLfloat, y: GLfloat, z: GLfloat) {
    with_ctx_and_mem(env, |gles, _mem| unsafe {
        gles.VertexAttrib3f(indx, x, y, z)
    })
}
fn glVertexAttrib4f(
    env: &mut Environment,
...
fn glVertexAttrib2fv(env: &mut Environment, indx: GLuint, values: ConstPtr<GLfloat>) {
    with_ctx_and_mem(env, |gles, mem| unsafe {
        gles.VertexAttrib2fv(indx, mem.ptr_at(values, 2))
    })
}
