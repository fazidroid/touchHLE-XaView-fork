...
fn objc_msgSend_inner(
    ...
    // ==========================================================
    // 1. NFS SHIFT 2: EA INFO & PATH RECOVERY (CRITICAL)
    // ==========================================================
    
    // Fix (null) dictionaries causing unexpected object panics
    if sel_str == "objectForInfoDictionaryKey:" {
        let val = crate::frameworks::foundation::ns_string::from_rust_string(env, "1.0".to_string());
        env.cpu.regs_mut()[0] = val.to_bits();
        return;
    }

    // Aggressively catch ALL path requests to prevent "(null)/Info.plist"
    if sel_str == "bundlePath" || sel_str == "resourcePath" || sel_str == "homeDirectory" || sel_str == "documentDirectory" || sel_str == "executablePath" {
    ...
    if sel_str == "uniqueIdentifier" || sel_str == "macaddress" {
    ...
    // ==========================================================
    // 3. NFS SHIFT 2: VIDEO & NETWORK KILL-SWITCH
    // ==========================================================
                        ...
                        if let Some((sent_type_id, _)) = message_type_info {
                            ...
                            if sent_type_id != expected_type_id && !tolerate_type_mismatch && 
                               sel_str != "bytes" && sel_str != "length" {
                                // Downgrade to a warning to prevent Shift 2 from panic crashing
                                println!("⚠️ WARNING: Type mismatch for {}", sel_str);
                            }
                        }
